...
namespace Businness.Concrete
{
    public class BaseCrud : IBaseCrud
    {
        ...
        public bool Execute(string sp, DynamicParameters parms, CommandType commandType = CommandType.StoredProcedure)
        {
            ...
            {
                return  db.Query<bool>(sp, parms, commandType: commandType).FirstOrDefault();
            };
        }
        ...
        public bool Insert<T>(string sp, DynamicParameters parms, CommandType commandType = CommandType.Text)
        {
            T result;
            using (IDbConnection db = new SqlConnection(_conf.GetConnectionString(Connectionstring)))
            {
        ...
        public bool Update<T>(string sp, DynamicParameters parms, CommandType commandType = CommandType.Text)
        {
            T result;
            using (IDbConnection db = new SqlConnection(_conf.GetConnectionString(Connectionstring)))
            {
